...
#
#     [주의] 여기서 "일부만 성공한 결과"를 부분 저장하지 않는 이유:
#     이 워크플로우는 여러 URL의 이미지를 한 줄로 이어붙인 뒤
#     valid_b_count 순번으로 아래 행들에 배분하는 구조라서,
#     URL 하나가 통째로 빠지면 이후 모든 행의 이미지가
#     한 칸씩 밀려서 엉뚱한 행에 들어갈 위험이 있다.
#     그래서 "전부 성공" 아니면 "아예 쓰지 않음"으로 안전하게 처리한다.
#
# [의존성] extract_images(url)이 이제
#     {"images": [...], "deleted": bool} 형태의 dict를 반환한다고 가정.
#     (regex_test.py 쪽도 이 형태 + 404 감지로 함께 수정 필요)
# -------------------------------------------------
...
#
# [주의] 이 검사 때문에, 실제로 이미지가 없는 정상적인 글도
# 매번 실패로 처리되어 시트가 영영 갱신되지 않을 수 있다.
# 정규식이 놓치는 게 아니라 정말로 이미지가 없는 글이라고 확인되면
# 이 로직을 다시 조정해야 한다.
# -------------------------------------------------
